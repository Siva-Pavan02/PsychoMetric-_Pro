...
export async function GET(_req: NextRequest) {
  ...
  const totalRevenuePaise = successfulPayments.reduce((sum: number, p: { amount: number }) => sum + p.amount, 0);
  ...
  return NextResponse.json({
    ...
    recentTransactions: successfulPayments.slice(0, 10).map((p: { amount: number, currency: string, createdAt: Date }) => ({
      amount:    p.amount / 100,
      currency:  p.currency,
